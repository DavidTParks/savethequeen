<template>
  <nav
    class="px-6 pt-6 text-base lg:text-sm lg:pl-6 lg:pr-8 sticky?lg:h-(screen-16)"
  >
    ...
    <template v-else-if="$fetchState.error">
      <h5
        class="mb-3 lg:mb-2 text-gray-500 uppercase tracking-wide font-bold text-base lg:text-xs"
      >
        Collections
      </h5>
    ...
    <template v-else>
      ...
      <ul class="mb-12">
        <template v-for="price in prices">
          <li class="mb-3 lg:mb-1" :key="price.id">
            <a
              @click="handlePriceSorting(price)"
              :class="[
                price.value === selectedPrice
                  ? 'text-red-600'
                  : 'text-gray-600 hover:text-gray-900'
              ]"
              class="px-2 -mx-2 py-1 transition duration-200 ease-in-out relative block font-medium cursor-pointer text-base"
            >
              <span
                :class="[
                  price.value === selectedPrice
                ...
                class="rounded absolute inset-0 opacity-25"
              ></span>
              <span class="relative"
                >${{ price.value }} - {{ price.size }}</span
              >
            </a>
          </li>
        </template>
      ...
      <ul>
        <template v-for="collection in collections">
          <li
            class="mb-3 lg:mb-1"
            v-if="excludedCollections.indexOf(collection.id) === -1"
            :key="collection.id"
          >
            <a
              @click="handleCollectionSorting(collection)"
              :class="[
